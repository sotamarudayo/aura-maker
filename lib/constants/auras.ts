...
/** シークレット判定・覚醒ビジュアルが有効になる票数 */
export const SECRET_UNLOCK_VOTES = 10;

export type AuraAwakening = {
  percent: number;
  hint: string;
  /** 票数が SECRET_UNLOCK_VOTES 以上で覚醒状態 */
  unlocked: boolean;
};
...
export const STANDARD_AURA_TYPES: AuraType[] = [
  ...
  },
  {
    ...
    name: "ミステリアスジェイドオーラ",
    archetypeName: "じわ惹き魔",
    catchCopy: "静かな緑の余韻が、言葉にしない魅力を漂わせる。",
    description:
      "あなたの纏う空気感は、落ち着きの中に惹力を秘めたムードメーカー。じわっと心に残ります。",
    gradient:
      "radial-gradient(circle at 35% 32%, #a7f3d0, transparent 38%), radial-gradient(circle at 72% 68%, #6ee7b7, transparent 42%), linear-gradient(135deg, #022c22 0%, #0f172a 100%)",
    palette: { a: "#a7f3d0", b: "#6ee7b7", c: "#34d399" },
  ...
  },
  {
    ...
    name: "ドリームチェイサーオーラ",
    archetypeName: "夢語り野郎",
    catchCopy: "高く昇る銀光が、野心と可能性を語り始める。",
    description:
      "あなたの纏う空気感は、未来志向の推進力。目標に向かう姿が周囲を鼓舞します。",
    gradient:
      "radial-gradient(circle at 30% 28%, #f8fafc, transparent 38%), radial-gradient(circle at 78% 42%, #93c5fd, transparent 44%), linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%)",
    palette: { a: "#f8fafc", b: "#93c5fd", c: "#cbd5e1" },
  ...
  },
  {
    ...
    catchCopy: "夜の静寂に溶ける、眠気と余白の月光。",
    description:
      "あなたの纏う空気感は、落ち着いた夜の詩人。マイペースで独自の世界観を持っています。",
    gradient:
      "radial-gradient(circle at 50% 38%, #e2e8f0, transparent 22%), radial-gradient(circle at 50% 38%, #94a3b8, transparent 38%), radial-gradient(circle at 28% 72%, #14532d, transparent 42%), radial-gradient(circle at 78% 78%, #1e293b, transparent 45%), linear-gradient(135deg, #020617 0%, #0f172a 100%)",
    palette: { a: "#e2e8f0", b: "#64748b", c: "#14532d" },
  ...
  },
  {
    ...
    name: "神話のインクオーラ",
    archetypeName: "語り部の神",
    catchCopy: "物語そのものが滲み出す、銀と墨の軌跡。",
    description:
      "あなたの纏う空気感は、人の人生を一章にしてしまう語り部。言葉の余韻が長く残ります。",
    gradient:
      "radial-gradient(circle at 28% 30%, #475569, transparent 38%), radial-gradient(circle at 72% 62%, #1e293b, transparent 44%), radial-gradient(circle at 50% 50%, #334155, transparent 52%), linear-gradient(135deg, #020617 0%, #0f172a 100%)",
    palette: { a: "#64748b", b: "#334155", c: "#1e293b" },
  ...
  },
  {
    ...
    catchCopy: "銀白の光に七色が走り、唯一無二のスペクトラムを放つ。",
    description:
      "あなたの纏う空気感は、ジャンルを超えたレジェンド。どんな属性とも共鳴する稀な存在です。",
    gradient:
      "radial-gradient(circle at 18% 22%, #f472b6, transparent 34%), radial-gradient(circle at 82% 28%, #67e8f9, transparent 34%), radial-gradient(circle at 50% 78%, #fde047, transparent 38%), radial-gradient(circle at 50% 50%, #e2e8f0, transparent 48%), linear-gradient(135deg, #0f172a 0%, #1e293b 100%)",
  ...
  },
  {
    ...
    catchCopy: "銀色の予兆が、運命のような説得力をまとう。",
    description:
...
export const SECRET_AURA_TYPES: AuraType[] = [
  {
    ...
    gradient:
      "radial-gradient(circle at 30% 30%, #312e81, transparent 28%), radial-gradient(circle at 70% 70%, #020617, transparent 40%), linear-gradient(135deg, #000000 0%, #0f0720 100%)",
    palette: { a: "#312e81", b: "#1e1b4b", c: "#020617" },
...
const DIAGNOSIS_BY_AURA: Partial<Record<string, AuraDiagnosisKit>> = {
  "chaos-neon": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。場が止まった瞬間、勝手にスイッチが入るタイプだ。友達目線では『いると楽しい』『空気が明るくなる』で固定されやすく、盛り上げは才能じゃなく日常のクセとして観測されている。リアクションとノリで場を支える習慣が、そのままこのオーラの核になっている。`,
    shadow: ({ a }) =>
      `「${a}」をフルスロットルにし続けると、静かな場で周囲が省エネ落ちする。潤滑油も過熱すると燃える。オフ日を作らない盛り上げは、ただの騒音になる。`,
    ecology: ({ a, b }) => ({
  ...
  "hentai-nebula": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が刺さっている。好奇心のブレーキが薄い『むっつりど変態』枠として観測された。友達からは『質問が深すぎる』『好きの解像度がバグってる』『でも話してて面白い』と同時に書かれやすく、雑談を一瞬で沼に引きずり込む。禁断の深掘りは事故じゃなく、このオーラの必殺技だ。`,
    shadow: () =>
      `相手の温度を見ずに禁断質問を投下すると、沼じゃなく穴になる。『この沼、一緒に入る？』の許可なしは、ただの襲撃だ。`,
    ecology: ({ a }) => ({
  ...
  "menhera-pulse": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。愛情確認と寂しさが表裏一体の『メンヘラかまってちゃん』としてロックされている。友達目線は『かまうと喜ぶ』『反応が遅いと天候が崩れる』『でも愛され方は上手い』。関係性の温度計を自分で持っているタイプで、既読の速度までオーラの一部になっている。`,
    shadow: () =>
      `確認が連続すると、相手の電池が先に切れる。不安は本音でも、理由なし連打は愛情じゃなく負荷だ。『なぜ疼いたか』を一言添えないと、かまってちゃんは消耗品になる。`,
    ecology: ({ a }) => ({
  ...
  "sunrise-hero": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が効いている。困った場と本番で前に出る太陽枠だ。友達からは『頼りになる』『主人公感がある』と見られ、自然と中心座標に置かれる。頼られるのは偶然じゃなく、光の向きが最初から外を向いている証拠。`,
    shadow: () =>
      `主役を降りられないと、休む許可が自分から消える。サブ役を選べない太陽は、周囲の期待で焼ける。`,
    ecology: ({ a }) => ({
  ...
  "healing-mint": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が目立つ。そばにいるだけで場の呼吸が深くなるタイプとして観測された。友達目線は『落ち着く』『話しやすい』『相談していい人』。空気清浄機枠は、派手な必殺技より持続回復で勝つ。愚痴と沈黙の受け皿が、このオーラの本体だ。`,
    shadow: () =>
      `優しさは武器だが、自分の不調を後回しにすると中から腐る。充電なしの癒やしは、ただの自己犠牲だ。頼まれる前に、自分のオフを確保できないと溶ける。`,
    ecology: ({ a }) => ({
  ...
  "gourmet-sun": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。食と笑いで人を巻き込む太陽だ。友達からは『一緒にいると元気が出る』『話題が尽きない』と書かれやすく、日常の幸福を拡散する装置として観測されている。飯テロは悪癖じゃなく、このオーラの攻撃範囲。`,
    shadow: () =>
      `陽気さが勝ちすぎると、疲れている相手には刺激になる。空腹と心の余白を同時に見られないと、太陽がただの騒音になる。`,
    ecology: ({ a }) => ({
  ...
  "soft-peach": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が効く。距離の縮め方が上手く、警戒を溶かす近接タイプだ。友達目線は『打ち解けやすい』『なんか安心する』。親しみは演技じゃなく、最初から温度が高い。少人数の雑談で本領を発揮し、会話が早く親密になる。`,
    shadow: () =>
      `近さは魅力だが、相手の更新速度を無視すると侵入になる。好意と遠慮を言語化できないと、安心が圧に変わる。`,
    ecology: ({ a }) => ({
  ...
  "mystic-purple": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。すぐ全部は見せない神秘枠としてロックされている。友達目線は『読めないけど惹かれる』『深そう』『考察したくなる』。沈黙と余白が武器で、情報を出し惜しみするほど印象が濃くなるタイプだ。`,
    shadow: () =>
      `閉じすぎると、神秘じゃなく壁になる。信頼できる相手に一枚もめくらないと、孤立と誤解が同時に育つ。謎は餌だが、餌なしは放置だ。`,
    ecology: ({ a }) => ({
  ...
  "dream-chaser": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が効いている。未来の話と目標で周囲を引っ張る推進枠だ。友達からは『夢がある』『前を向いてる』と見られ、話を聞く側の野心まで点火する。今より先を語る癖が、このオーラのエンジン。`,
    shadow: () =>
      `理想が強いと、今ここを軽く見られた感じが残る。足元の小さな達成を祝えない夢語りは、ただの先延ばしに見える。`,
    ecology: ({ a }) => ({
  ...
  "electric-cyan": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。頭の回転と切れ味で場を動かすタイプだ。友達目線は『鋭い』『話が早い』『論点が見える』。議論と企画で存在感が立ち、雑談ですら解像度が上がる。速さは性格じゃなく、このオーラの攻撃速度。`,
    shadow: () =>
      `切れ味が勝ちすぎると、感情の話で刺さる。正しさ優先のまま雑談に入ると、頭回り最速がただの冷たい人になる。温度を落とすモードがないと耐久が死ぬ。`,
    ecology: ({ a }) => ({
  ...
  "imp-neon": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が同居している。見せ方と中身の落差で人を惹きつけるギャップ型だ。友達からは『予想外が面白い』『一筋縄ではいかない』と観測され、油断した瞬間に本性が顔を出す。ギャップは演出じゃなく、このオーラの基本攻撃。`,
    shadow: () =>
      `演じ続けすぎると、本音の置き場がなくなる。安心できる相手の前でもギャップを消さないと、小悪魔が自分を食う。`,
    ecology: ({ a }) => ({
  ...
  "midnight-moon": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。夜と余白で本領を発揮するタイプだ。友達目線は『マイペース』『夜に強い』『静かな引力がある』。昼間は薄く、深夜に濃くなる。語彙と感性が夜モードに切り替わる瞬間が、このオーラの覚醒ポイント。`,
    shadow: () =>
      `夜型の魅力は強いが、朝の自分との落差で誤解される。取扱説明書なしの夜更かし詩人は、ただの連絡つかない人になる。`,
    ecology: ({ a }) => ({
  ...
  "otaku-galaxy": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が強い。好きへの熱量が銀河級として観測された。友達からは『推しが尊い』『好きなものの話が面白い』と書かれ、同好が集まると一気に中心になる。ニッチへの愛は趣味じゃなく、このオーラの燃料だ。`,
    shadow: () =>
      `熱量が勝ちすぎると、興味のない相手の参加権が消える。入口の共有なしで沼に落とすと、推し活が排他になる。`,
    ecology: ({ a }) => ({
  ...
  "crimson-rebel": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。枠を壊す熱と反骨で場を塗るタイプだ。友達目線は『カッコいい』『危ないけど魅力的』。理不尽な空気に火がつきやすく、常識ゲージを揺らすのがこのオーラの仕事。`,
    shadow: () =>
      `壊す対象を選ばないと、反骨がただの衝突に見える。何に怒るかのセンスがないと、番長がただの迷惑になる。`,
    ecology: ({ a }) => ({
  ...
  "velvet-muse": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が効く。雰囲気そのものが作品になるミューズ型だ。友達からは『なんか綺麗』『余韻が残る』と観測され、言葉より空気で人を動かす。説明しなくても印象が残るのが、このオーラの特殊能力。`,
    shadow: () =>
      `雰囲気任せだと、意図が伝わらず距離が空く。素の一言を足せないミューズは、綺麗な壁で終わる。`,
    ecology: ({ a }) => ({
  ...
  "mythic-quill": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。語りと解釈で世界を編むタイプだ。友達目線は『話がうまい』『見方が面白い』。場の意味づけ役になりやすく、雑談が長編化する。事実より物語で残すのが、このオーラの記録方式。`,
    shadow: () =>
      `語りが上手いぶん、事実より物語が先行して誤解を生む。聞き役を持たない語り部は、信頼よりエンタメで消費される。`,
    ecology: ({ a }) => ({
  ...
  "legendary-prism": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が同時に強い。複数の光が重なるレア枠だ。友達からは『何者なんだろう』『総合力が高い』と見られ、印象が立体的に観測される。一つの属性に収まらないのが、このオーラの伝説理由。`,
    shadow: () =>
      `全部を同時に出すと焦点がぼやける。今日はどの面を表にするか選べないと、全属性がただの未整理になる。`,
    ecology: ({ a, b }) => ({
  ...
  "golden-oracle": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が効いている。助言と判断で周囲を導くタイプだ。友達目線は『この人に聞くと安心』『視点が一段高い』。相談が集まりやすく、不安を整理して行動指針に変えるのがこのオーラの仕事。`,
    shadow: () =>
      `常に正解を出す役は重い。「わからない」を言えないオラクルは、賢者じゃなく解答機になる。頼られすぎると、自分の判断まで削れる。`,
    ecology: ({ a }) => ({
  ...
  "void-abyss": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。存在感と不在感が同時に立つヴォイド型だ。友達目線は『いるのに読めない』『静かな重力がある』。大人数の端でも観測され、沈黙そのものがオーラになっている。`,
    shadow: () =>
      `沈黙は魅力だが、合図ゼロだと忘れられた感覚に変わる。小さな存在確認を残さないヴォイドは、深淵じゃなく欠席になる。`,
    ecology: ({ a }) => ({
  ...
  "phantom-mirror": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が同居している。相手と場面で別人に見えるミラー型だ。友達からは『状況で顔が変わる』『合わせ方が上手い』と観測され、ギャップが話題になる。切り替えは演技というより、このオーラの反射神経。`,
    shadow: () =>
      `合わせ上手は強みだが、自分の輪郭が薄くなる。誰の前でも消えない一点を持たないと、別人スイッチが自己消失になる。`,
    ecology: ({ a, b }) => ({
  ...
  "god-calamity": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}のような相反が上位。規格外の天変地異タイプだ。友達目線は『予想不能』『いると何か起きる』。ヒーローとカオスが同時発火し、常識が一時停止する。破壊力は事故じゃなく、このオーラの仕様。`,
    shadow: () =>
      `発動条件を自分で把握できないと、伝説じゃなく災害になる。日常の説明責任から逃げる天変地異は、ただの迷惑だ。`,
    ecology: ({ a, b }) => ({
  ...
  "absolute-crystal": {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。触れにくさと美しさが同居する結晶タイプだ。友達目線は『近づきにくい』『でも目が離せない』。距離そのものが防壁で、クリアな空気の中で研ぎ澄まされる。冷たさは拒絶じゃなく、このオーラの硬度。`,
    shadow: () =>
      `防壁は好意も弾く。温度を少しだけ通す許可を出せないと、結晶はただの氷で終わる。触れがたいは強みだが、永遠に触れられないと関係が死ぬ。`,
    ecology: ({ a }) => ({
...
function fallbackDiagnosisKit(aura: AuraType): AuraDiagnosisKit {
  return {
    reading: ({ a, b }) =>
      `${pairLabel(a, b)}が上位。${aura.archetypeName}としての輪郭がはっきり観測されている。友達目線は『${aura.catchCopy}』に近く、日常のクセの積み重ねがこの結果になった。`,
    shadow: () =>
      `『${aura.archetypeName}』を出し続けると、周囲が疲れるか離れる。強みのオフ日を作らないと、印象の寿命が先に尽きる。`,
    ecology: ({ a, b }) => {
...
function buildAwakening(votes: string[]): AuraAwakening {
  ...
  } else if (total < SECRET_UNLOCK_VOTES) {
    hint = `あと${SECRET_UNLOCK_VOTES - total}票でオーラ覚醒（シークレット判定が開く）`;
  } else if (unique < 8) {
    hint = "覚醒済み。いろんな印象ワードが集まるとシークレットの兆し";
  } else {
    hint = "覚醒済み。シークレット条件に近い組み合わせかも…？";
  }

  return { percent, hint, unlocked };
}
...
const SECRET_FLAVOR =
  "投票が10票以上集まると判定が開く、幻のオーラ。条件は明かされない。";
...
export function calculateAuraType(
  ...
  // 10票未満は通常オーラのみ。シークレットは覚醒後にのみ判定。
  if (votes.length >= SECRET_UNLOCK_VOTES) {
...
export function getAuraCatalogEcology(auraId: string): {
  habitat?: string;
  trigger?: string;
  sideEffect?: string;
  weakness?: string;
  ...
  return {
    habitat,
    trigger: ecology?.trigger,
    sideEffect: ecology?.sideEffect,
